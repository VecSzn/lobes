...
IMG = ROOT / "docs" / "img"
# items per suite; a suite is drawn only once a run has all of them.
N = {"gsm8k": (100,), "ifeval": (100,), "mbppplus": (100,), "gpqa": (100,)}
# label, results directory, condition. The first two share a solver, so their difference is the relay's.
# A missing file is skipped.
SERIES = [("bare Qwen3.5-4B", "bare4b-0919", "bare-4b"),
          ("Lobes, medium", "final-0919", "specialists"),
          ("bare Qwen3.5-9B", "fix-0919", "bare-9b")]
COLORS = ["#7a7a7a", "#4c72b0", "#dd8452"]


def load(tag, cond):
    p = ROOT / "eval" / "results" / tag / f"{cond}-s0.jsonl"
    seen = {}      # first row per item: a relaunched run appends a second row for items it redid
...
def cost():
    ...
    for ax, key, title in ((axes[0], lambda r: r["tokens"].get("total_tokens", 0), "tokens per item"),
        ...
        for i, (label, d) in enumerate(runs):
            # median, not mean: a handful of items that run to the cap otherwise set the bar height
            v = statistics.median(key(r) for s in shared for r in d[s])
        ...
        ax.spines[["top", "right"]].set_visible(False)
    fig.suptitle(f"median over {', '.join(shared)}  (lower is better)", fontsize=8)
    fig.tight_layout()
    return fig
