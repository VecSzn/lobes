...
**它会在你机器上做什么。** 运动叶回答任何能算的题，靠的是挑一个工具跑起来：一个 python
子进程、一条 shell 命令、在本次任务工作目录里读写文件、抓一个 url、截一张屏。python 和
shell 这两个直接执行模型写出来的东西，用你的身份，除了 10 秒超时没有别的拦着——**没有沙箱**，
而且这正是设计本身：程序跑过、打印出答案，这个答案才算数。文件工具锁在工作目录里，python
和 shell 不锁。这台机器要是你不愿意交给一个 3B 模型，就把整个进程放进容器里跑。
