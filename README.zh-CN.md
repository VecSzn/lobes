...
flowchart TD
  ...
  E -->|跑程序有用| R["推理叶，先不思考：按题问的顺序列出每个值，再交一段逐行打印它们的复算程序"]
  R -->|题要的是源码| C["就交那段代码，题目自带的例子或一份盲写的测试跑它，失败带着原因重做一次"] --> L
  R -->|值| M["运动叶：只看题目写一段程序，程序输出就是它的值"] --> A{"逐个值一致？两个即可，没跑过程序就要全部"}
  E -->|闭卷| Q["推理叶不思考答 3 次（高档 5 次），互相看不见"] --> A
  A -->|不一致，还有证人| V["再叫一个：验证叶换一个家族盲解，之后才让推理叶开思考，再答 3 次（高档 5 次）"] --> A
...
中间量的只比末尾）。两个一致就定案：有一个跑过程序叫 `evidence`，都没跑叫 `consistency`；
闭卷题要连续三个样本一致。便宜的证人不一致才开思考。程序挂了只带着 stderr 修一次，仅此而已。`--effort low|medium|high|xhigh|max|auto` 一起调
